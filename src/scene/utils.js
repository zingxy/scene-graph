...
export function addDragBehavior(circle) {
  ...
  circle.on('global:pointermove', (event) => {
    if (!isDragging) return;
    console.log('@@drag move', event);
    const dx =
      circle.parent.worldToLocal(event.worldPoint).x -
      circle.parent.worldToLocal(lastPosition).x;
    const dy =
      circle.parent.worldToLocal(event.worldPoint).y -
      circle.parent.worldToLocal(lastPosition).y;
    circle.x += dx;
    circle.y += dy;
    lastPosition = event.worldPoint;
  });
...
globalThis.decompose = decompose;
